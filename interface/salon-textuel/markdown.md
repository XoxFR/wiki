...
## Les blocs de code
Il est aussi possible de formater du texte sous forme de code. Il suffit de mettre \`\`\` devant le code puis le langage (exemples : `js`, `java`, `css`, `html`), ensuite il faut revenir à la ligne et taper le code, enfin, il est nécessaire fermer votre code de la même manière \`\`\`. 
